...
export function createRegisterXProtocol(funcName) {
  return (name, handle) =>
    new Promise((resolve, reject) =>
      protocol[funcName](name, handle, (error) =>
        error ? reject(error) : resolve(),
      ),
    )
}

export const registerFileProtocol = createRegisterXProtocol(
...
export function resolveActualPathFromURI(
  uri,
  { protocolName = 'file', omitDeviceLetter = false } = {},
) {
  const rawPath = resolvePathFromURI(uri, protocolName)
  const startWithLength =
    process.platform === 'win32' ? 1 + (omitDeviceLetter ? 2 : 0) : 0
